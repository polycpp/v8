...
def run_test(d8_path, test_path, mjsunit_js, v8_root, timeout):
    ...
    # Skip tests that use flags not present in this V8 version.
    bad_flags = [f for f in flags if f.split("=")[0] in NONEXISTENT_FLAGS]
    if bad_flags:
        test_name = os.path.relpath(test_path, os.path.join(v8_root, "test", "mjsunit"))
        return test_name, None, 0.0, f"SKIP (nonexistent flags: {' '.join(bad_flags)})"
