...
def main():
    ...
    # Auto-detect ICU major version from uvernum.h
    icu_version = 74  # default fallback
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    uvernum_paths = [
        os.path.join(project_root, 'v8-src', 'third_party', 'icu', 'source', 'common', 'unicode', 'uvernum.h'),
        os.path.join(os.path.dirname(input_path), '..', 'source', 'common', 'unicode', 'uvernum.h'),
    ]
    for uvernum_path in uvernum_paths:
        if os.path.exists(uvernum_path):
            import re
            with open(uvernum_path, 'r') as uf:
                for line in uf:
                    m = re.match(r'#define\s+U_ICU_VERSION_MAJOR_NUM\s+(\d+)', line)
                    if m:
                        icu_version = int(m.group(1))
                        break
            break
