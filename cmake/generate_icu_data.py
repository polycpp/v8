...
def detect_icu_version(dat_path):
    """Detect ICU major version from uvernum.h or the dat file header."""
    import re
    # Try to find uvernum.h relative to the dat file
    # dat is typically at third_party/icu/common/icudtl.dat
    icu_dir = os.path.dirname(os.path.dirname(dat_path))  # up to third_party/icu
    uvernum = os.path.join(icu_dir, "source", "common", "unicode", "uvernum.h")
    if os.path.exists(uvernum):
        with open(uvernum, "r") as f:
            content = f.read()
        m = re.search(r'#define\s+U_ICU_VERSION_SHORT\s+"(\d+)"', content)
        if m:
            return m.group(1)
    # Fallback: read the dat file header (first ~20 bytes contain version info)
    with open(dat_path, "rb") as f:
        header = f.read(32)
    # The dat header typically starts with "icudt<VER>l" in little-endian
    m = re.search(rb'icudt(\d+)', header)
    if m:
...
def main():
    ...
    # Auto-detect ICU version from the dat file header or nearby source
    icu_version = detect_icu_version(input_path)
    symbol_name = f'icudt{icu_version}_dat'
