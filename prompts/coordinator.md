...
1. Create branch `v8-{TARGET_VERSION}` from `dev`
2. Run `scripts/generate_fetch_deps.py` to create `fetch_deps.py`
3. Execute `fetch_deps.py` to download V8 source and dependencies
4. Run `scripts/detect_features.py` to produce `version_features.json`
5. **Review**: Verify feature manifest makes sense for this V8 version

### Phase 2: Source Generation
1. Run `scripts/generate_sources.py` to produce `cmake/sources.cmake`
2. Run `scripts/generate_torque_list.py` to produce torque file list
3. Generate `CMakeLists.txt` from template using feature manifest
4. Copy stable cmake modules from `cmake/` directory
5. **Review**: Spot-check generated source lists against V8 source tree
   - Are key directories represented? (src/compiler, src/heap, src/objects, etc.)
   - Do conditional features match the feature manifest?

### Phase 3: Build Iteration (the core loop)
```
while build fails:
    1. Attempt build: cmake -B build -G Ninja && cmake --build build
